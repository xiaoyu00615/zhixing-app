...
function mapClientError(
  ...
  if (error instanceof SearchWorkerClientError) {
    switch (error.code) {
      case 'INVALID_QUERY':
      case 'PERSISTENCE_ERROR':
        return new SearchRepositoryError(error.code, operation)
    }
  }
  return new SearchRepositoryError('PERSISTENCE_ERROR', operation)
}
...
export class WebSearchRepository implements SearchRepository {
  ...
  async query(input: SearchQuery): Promise<SearchResult[]> {
    ...
    try {
      const value = await this.#client.searchQuery({ query, limit })
